...
class TrainProcessor(object):
    ...
    def training(self, sess, train_writer):
        ...
        with tf.name_scope('debug_board'):
            res_map = tf.cast(tf.reshape(tf.argmax(self.net.predicted_map, axis=3), [-1, 640, 640, 1]),
                              dtype=tf.float32)
            ...
            tf.summary.image('FinalResult', final_res)
        ...
        with tf.name_scope('epoch_valid'):
            ...
            epoch_valid_loss_sum_op = tf.summary.scalar('valid_los', epoch_valid_loss)
        ...
        for epo_cnt in range(self.args.epoch_iters):
            for data_idx in training_series:  # DO NOT EDIT the "training_series",for the latter shuffle
                ...
                timer.tic()
                res_map_, loss_, merged_, _ = sess.run([res_map, loss, merged, train_op], feed_dict=feed_dict,
                ...
                if iter % 20 == 0 and cfg.TRAIN.TENSORBOARD:
                    ...
                    pass
                if (iter % 4000 == 0 and cfg.TRAIN.DEBUG_TIMELINE) or (iter == 20):
                    # chrome://tracing
                ...
                if DEBUG:
                    ...
                    pcd_vispy(scan, boxes=boxes, name='CubicNet training', index=iter, vis_size=(800, 600),
